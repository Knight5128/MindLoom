...
- 主页面克制、有高级感的灰暗色调
- 冷雾 / 低蓝光烛光双主题，可即时切换并持久保留
- 编辑器字号（16/18/20px）与行宽（560/640/760px）均可调并持久保留
- 晚安收笔仪式会先保存，再展示今晚字数与连续记录天数，最后隐藏到托盘
- UI 默认完全隐藏，光标移近窗口边缘时浮现（顶部 / 左侧 / 底部）
- 打字机模式自动让当前光标行保持在屏幕中部；输入时隐藏指针和边缘 UI，移动鼠标恢复
- 6 种可切换背景：星空、雨夜、极光、雾气、纯色渐变、4-7-8 呼吸圆环
- 4 段程序化合成的环境音：雨声、风声、白噪、钵音（无外部资源依赖）
- 4 组场景预设可一键联动背景与环境音，呼吸场景可同步钵音并显示相位倒计时
- 15 / 30 分钟助眠定时器会在最后 60 秒渐弱环境音并自动停止
- 启动问候随时段变化，编辑器引导语在新草稿时温和轮换
- 历史笔记支持按内容即时搜索
...
| `Ctrl/Cmd + N` | 新建一条笔记 |
| `Ctrl/Cmd + S` | 立即落盘 |
| `Ctrl/Cmd + Enter` | 保存并启动「晚安」收笔仪式 |
| `F11` | 进入 / 退出全屏沉浸模式 |

## 窗口行为

- 点击标题栏 ✕：**先落盘，再隐藏到系统托盘**（应用继续驻留，点托盘图标唤回）
- 真正退出：托盘菜单「退出 MindLoom」（退出前自动落盘未保存内容）
...
- 底部 Dock「数据」组提供：导出今日 MD / 全部 JSON、从 JSON 备份导入、打开数据文件夹。
...
- 遵循系统“减弱动态”偏好：canvas 背景仅绘制单帧，功能性的 4-7-8 呼吸环保留
